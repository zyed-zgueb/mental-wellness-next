...
- **AI Integration**: Vercel AI SDK with OpenRouter integration (access to 100+ models)
...
**IMPORTANT**: This project uses **OpenRouter** (not direct OpenAI) to access 100+ AI models through a single API. Always use the `OPENROUTER_MODEL` environment variable for the model name instead of hardcoding it:

```typescript
// ✓ Correct - Use environment variable with OpenRouter
...
This allows for easy model switching without code changes and ensures consistency across the application. Users can browse available models at https://openrouter.ai/models.
...
Je souhaite développer une application Web SaaS de suivi de ma santé mentale personnelle et de mon bien-être.
